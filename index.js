  ...
  const invoiceData = {
   faturaUuid: invoiceDetails.uuid || uuid(),
    ...
    malHizmetTable: invoiceDetails.items.map(item => ({
      ...
      malHizmetTutari: item.totalServicePrice ? item.totalServicePrice.toFixed(2).toString() : ((item.quantity || 0) * (item.unitPrice || 0)).toFixed(
        2
      ).toString(),
    ...
    not: invoiceDetails.note ? invoiceDetails.note : convertPriceToText(invoiceDetails.paymentTotal)
